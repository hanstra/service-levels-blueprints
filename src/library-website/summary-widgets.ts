import { Metric, GraphWidget, Statistic, LegendPosition, MathExpression } from "@aws-cdk/aws-cloudwatch";
...
const apis = [
  { apiName: "contentfuldirect-prod", stage: "prod", label: "Content API", color: "#ff7f0e" },
  { apiName: "contentfulmaps-prod", stage: "prod", label: "Maps API", color: "#2ca02c" },
  { apiName: "recommendEngine-prod", stage: "prod", label: "Directory API", color: "#d62728" },
  { apiName: "monarchLibguides-prod", stage: "prod", label: "Hours API", color: "#9467bd" },
  { apiName: "gatekeeper-prod", stage: "prod", label: "GateKeeper API", color: "#8c564b" },
  { apiName: "classesAPI-prod", stage: "prod", label: "Classes API", color: "#e377c2" },
  { apiName: "userPreferences-prod", stage: "prod", label: "User Preferences API", color: "#7f7f7f" },
]
const lambdas: string[] = [
  "classesAPI-prod-passthrough",
  "contentfuldirect-prod-archivequery",
  "contentfuldirect-prod-directquery",
  "contentfuldirect-prod-purlquery",
  "recommendEngine-prod-librarianInfo",
...
const cdnRequestMetrics = metrics.cloudfrontRequests({ distributionId: cdnDistributionId, label: "HTTP CDN", color: "#1f77b4" });
...
const apiErrorRates = apis.map((api, i) => metrics.apiErrors({ ...api, metricIdx: i }).rate5xx);
const cdnErrorsMetrics = metrics.cloudfrontErrors({ distributionId: cdnDistributionId, label: "HTTP CDN", color: "#1f77b4" }).rate5xx;
...
const cdnLatency = metrics.cloudfrontLatency({ distributionId: cdnDistributionId, label: "HTTP CDN (Cache Miss Only)", color: "#1f77b4" });
...
const concurrentExecutions = lambdas.map(lambda => metrics.lambdaConcurrentExecutions(lambda));
